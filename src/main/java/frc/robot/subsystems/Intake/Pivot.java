...
public class Pivot extends SubsystemBase {
    private final ArmFeedforward m_feedforward = new ArmFeedforward(PIDConstants.kS, PIDConstants.kG, PIDConstants.kV, PIDConstants.kA);

    private SmartMotorControllerConfig smcConfig = new SmartMotorControllerConfig(this)
    .withControlMode(HardwareConstants.kControlMode)
    // Feedback Constants (PID Constants)
    .withClosedLoopController(PIDConstants.sparkP, PIDConstants.sparkI, PIDConstants.sparkD, ProfileConstants.kSparkMaxVelocity, ProfileConstants.kSparkMaxAcceleration)
    .withSimClosedLoopController(PIDConstants.sparkP, PIDConstants.sparkI, PIDConstants.sparkD, ProfileConstants.kSparkMaxVelocity, ProfileConstants.kSparkMaxAcceleration)
    // Feedforward Constants
    // Telemetry name and verbosity level
    .withTelemetry(TelemetryConstants.kMotorName, TelemetryConstants.kTelemetryVerbosity)
    .withGearing(new MechanismGearing(GearBox.fromReductionStages(MechanismConstants.kGearRatioStage1, MechanismConstants.kGearRatioStage2)))
    ...
    private final ProfiledPIDController m_controller =
        new ProfiledPIDController(
            ...
            PIDConstants.rioI,
            PIDConstants.rioD,
            new TrapezoidProfile.Constraints(ProfileConstants.kRioMaxVelocity, ProfileConstants.kRioMaxAcceleration));
    private SparkMax spark = new SparkMax(HardwareConstants.canId, HardwareConstants.motorType);
    private SmartMotorController sparkSmartMotorController = new SparkWrapper(spark, HardwareConstants.motor, smcConfig);
    ...
    public Command setAngle(Angle angle) { return setAngleWithRioProfile(angle);}
    ...
    public Command setAngleWithRioProfile(Angle angle) {
        return run(() -> {
            ...
            double ffOutput = m_feedforward.calculate(m_controller.getSetpoint().position, m_controller.getSetpoint().velocity);
